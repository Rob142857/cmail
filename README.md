...
**Hosting cost:** Cloudflare Pages + Email Routing + D1 + R2 fit comfortably inside Cloudflare's free tier for small orgs — no Workers Paid plan required. Outbound to external recipients is delivered through a third-party transactional relay (Resend or Postmark); see [Outbound providers](#outbound-providers) for free-tier limits and pricing.
...
cmail auto-detects which outbound provider to use based on which env vars are configured. Priority order: **Resend → Postmark → none**. Pick one.

| Provider | Env var | Free tier | Paid pricing | Best for |
|----------|---------|-----------|--------------|----------|
| **Resend** | `RESEND_API_KEY` | 3,000 emails/month, 100/day, 1 verified domain | Pro $20/mo (50k emails); Scale $90/mo (100k); $1/1k overage | General outbound to any recipient. Recommended default. |
| **Postmark** | `POSTMARK_API_KEY` | 100 emails/month (test mode — requires sender approval before sending to unverified recipients) | $15/mo (10k emails); pay-as-you-go from $1.25/1k | Strict-deliverability transactional mail. |

> 💡 **Why not Cloudflare's native `send_email` binding?** Cloudflare's `send_email` Workers binding can only deliver to email addresses that have been verified as destination addresses in your Email Routing zone. It is not a general-purpose SMTP relay, so it doesn't fit cmail's use case (sending to arbitrary external recipients). cmail uses Cloudflare for inbound (Email Routing → Worker) and hosting (Pages + D1 + R2), and a third-party transactional service for outbound.
...
If no outbound provider is configured, cmail runs in **inbound + internal only** mode (users can receive external email but the compose UI shows a banner and outbound is disabled).
