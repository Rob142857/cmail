...
cmail processes authentication data, message content, attachments, and administrative records. Security reports are taken seriously, but this pre-1.0 project has not been certified for any particular regulatory or assurance framework.

## Supported versions

Until versioned releases are published, security fixes target the latest revision of the default branch. Older commits, forks, and deployment-specific modifications are not supported by the project.
...
If private reporting is unavailable, ask the maintainers for a private contact channel without publishing technical details. Do not open a public issue containing an exploit, secret, personal data, private message, tenant identifier, or production URL.
...
Maintainers aim to acknowledge a complete report within seven days. Investigation and remediation time depends on severity and complexity. Please allow a coordinated fix and disclosure before publishing details.
...
Provider outages, provider-account compromise, DNS misconfiguration, unsupported local changes, and social engineering without a product flaw are normally deployment concerns rather than project vulnerabilities. They can still be reported privately if the boundary is unclear.
...
If credentials are exposed, rotate or revoke them immediately. Removing a file from the latest commit is not sufficient when the value exists in Git history, build logs, caches, or deployed configuration.
...
printing secret values. A failing history check is a release blocker even when
