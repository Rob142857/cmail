...
import { isAllowedPushEndpoint, sendTestPushNotification } from '@cmail/shared/push';
...
function response(result: string, status: number, extraHeaders: Record<string, string> = {}) {
  return json({ result }, { status, headers: { ...RESPONSE_HEADERS, ...extraHeaders } });
}
...
export const POST: RequestHandler = async ({ request, locals, platform }) => {
  ...
  if (!env) return response('configuration', 503);
  ...
  try {
    ...
    return response('transient', 503);
  }
  if (!rate.allowed) return response('rate_limited', 429, { 'Retry-After': String(rate.retryAfter) });
  ...
  if (summary.configuration > 0) return response('configuration', 503);
  if (summary.retryable > 0) return response('transient', 503);
